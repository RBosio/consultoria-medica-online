...
export class SpecialityService {
    ...
    constructor(@InjectRepository(Speciality) private specialityRepository: Repository<Speciality>) {}
    ...
    async findOne(id: number): Promise<Speciality | HttpException> {
        ...
        if (!specialityFound) {
            throw new HttpException('Especialidad no encontrada', HttpStatus.NOT_FOUND)
        }
        
        return specialityFound
    }

    async create(speciality: createSpecialityDto): Promise<Speciality | HttpException> {
        const specialityFound = await this.specialityRepository.findOne({
            where: {
