...
const Layout: React.FC<LayoutProps> = ({ children, renderNavbar = true, renderSidebar = true, className, auth, contentRef, navbarLeftElement }) => {
    ...
    return (
        <main className={`h-full ${roboto.className} bg-slate-200 flex`}>
            ...
            <section className="grow w-full overflow-hidden flex flex-col">
                {renderNavbar && <Navbar leftElement={navbarLeftElement} renderSidebar={renderSidebar} auth={auth} sidebarOpened={sidebarOpened} setSidebarOpened={setSidebarOpened}/>}
