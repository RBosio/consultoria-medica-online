...
const Reports = (props: AnalyticsProps) => {
  ...
  const reports: Report[] = [
    {
      ...
      description:
        "Este reporte muestra las reuniones que tuvo un médico/varios médicos para un determinado período y obra social",
    ...
    },
    {
      ...
      description:
        "Este reporte muestra los pagos que se le realizaron a los profesionales médicos para todos los períodos disponibles con su respectivo monto",
  ...
  const renderReport = () => {
    ...
    return React.cloneElement(
      reportsElements[selectedReport.id as keyof typeof reportsElements],
    ...
    <>
      <TableContainer className="h-full" component={Paper}>
        <Table aria-label="reports table">
          ...
          <TableBody>
            {reports
              .filter((row) => row.roles.includes(props.auth.role as any))
              .map((row, idx) => (
                <TableRow
                  key={idx}
                  ...
                  >
                    <div className="flex items-center gap-1">
                      ...
                      {row.input_needed ? (
                        ...
                        <Link
                          className="cursor-pointer"
                          href={`${process.env.NEXT_PUBLIC_API_URL}/${row.path}`}
                          target="_blank"
                        >
                          {row.name}
                        </Link>
...
const MeetingReport: React.FC<{
  ...
  useEffect(() => {
    async function fetchData() {
      let drs = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/doctor/`, {
        withCredentials: true,
        headers: { Authorization: `Bearer ${props.auth.token}` },
      });
  ...
  const getReportPath = () => {
    ...
    path += `?month=${date.month() + 1}&year=${date.year()}${
      doctor ? `&userId=${doctor.id}` : ""
    }${healthInsurance ? `&hi=${healthInsurance.id}` : ""}`;
    ...
    <>
      <div className="flex flex-col gap-5">
        {props.auth.role === "admin" && (
          <Autocomplete
            ...
            renderInput={(params: any) => (
              <Input {...params} label="Médico" variant="outlined" />
            )}
          />
        )}
        <DatePicker
        ...
        <Autocomplete
          ...
          renderInput={(params: any) => (
            <Input {...params} label="Obra social" variant="outlined" />
          )}
        />
        ...
        <a className="self-end mt-4" href={getReportPath()} target="_blank">
  ...
  return (
    <div className="flex flex-col">
      ...
      <a
        ...
        href={`${process.env.NEXT_PUBLIC_API_URL}/${report?.path}${
          !complete ? `?month=${date.month() + 1}&year=${date.year()}` : ""
        }`}
        target="_blank"
      >
        <Button>Generar</Button>
