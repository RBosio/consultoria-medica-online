...
export default function Doctor(props: any) {
    ...
    const getDiscount = () => {
        ...
        const foundHealthInsurance = doctorsWorkingFor.filter((hi: any) => hi.id === userHealthInsurance.id);
            ...
            <section className="h-full flex overflow-y-auto xl:p-8">
                <div className="w-full flex flex-col xl:flex-row xl:gap-6 xl:mt-[3rem]">
                                    ...
                                    <div className="flex flex-col gap-2">
                                        ...
                                        <div className="flex flex-col gap-2">
                                            <div className="flex gap-2 text-primary items-center font-bold">
                                                <IoMdMail size={15} />
                                                <p className="text-secondary">{props.doctor.user.email}</p>
                                            </div>
                                            <div className="flex gap-2 text-primary items-center font-bold">
                                                <FaPhone size={15} />
                                                <p className="text-secondary">{props.doctor.user.phone}</p>
                                            </div>
                                            <div className="flex gap-2 text-primary items-center font-bold">
                                                <FaSuitcaseMedical size={15} />
                                                <p className="text-secondary">{props.doctor.user.healthInsurances.map((hi: any) => hi.name).join(" | ")}</p>
                                            </div>
                                            <div className="flex gap-2 text-primary items-center font-bold">
                    ...
                    <div className="flex flex-col bg-white p-4 gap-2 shadow-md grow xl:rounded-md relative">
                        <div className="w-full h-32 flex items-center justify-center bg-primary absolute right-0 top-0 xl:w-56 xl:h-56 rounded-tr-md w-full">
                            ...
                            <div className="flex flex-col text-white w-full h-full py-2">
                                ...
                                <div className="flex flex-col gap-1 grow items-center justify-center xl:mb-5">
                                    {Boolean(getDiscount()) ? <div className="flex gap-1 items-center">
                                        <p className={`text-lg text-primary_light line-through`}>{pesos.format(props.doctor.priceMeeting)}</p>
                                        <Chip
                                            className="mx-1 border-white text-white"
                                            size="small"
                                            variant="outlined"
                                            icon={<CiDiscount1 size={20} color="#ffffff" />}
                                            label={`${Math.floor(Number(getDiscount().discount) * 100)}% (${getDiscount().name})`}
                                        />
                                    </div> : null}
                                    <p className={`text-3xl ${robotoBold.className}`}>{getDiscount() ?
                                        pesos.format(props.doctor.priceMeeting * (1 - Number(getDiscount().discount)))
                                        :
                                        pesos.format(props.doctor.priceMeeting)}</p>
                                </div>
                            </div>
                        </div>
                                ...
                                {props.doctorAvailability.map((da: any) => {
                                    ...
                                    return (
                                        <div key={da.date}>
                                            ...
                                            {
                                                da.schedule.filter((avSch: any) => avSch.available).length > 0 ?
                                                    <ToggleButtonGroup className="flex-wrap" exclusive onChange={handleDateChange} value={selectedDate} size="small" aria-label="Small sizes">
                                                        {da.schedule.filter((avSch: any) => avSch.available).map((sch: any) => (
                                                            <ToggleButton sx={{
                                                    ...
                                                    :
                                                    <span className="text-red-600">No hay horarios disponibles</span>
                                            }
                                        </div>
                                    )
                                })}
                ...
                >
                    <DialogTitle className={`${robotoBold.className} text-primary`} id="alert-dialog-title">
                        Confirmar turno
                    </DialogTitle>
                    <DialogContent>
