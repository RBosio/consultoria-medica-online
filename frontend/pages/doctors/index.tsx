...
export default function Doctors(props: any) {
  ...
  const onPageChange = async (ev: React.ChangeEvent<unknown>, page: number) => {
    const queryWPage = new URLSearchParams({
      ...router.query,
      page: page.toString(),
    }).toString();
    ...
    <Layout auth={props.auth}>
      <section className="h-full flex flex-col xl:flex-row overflow-hidden">
        <Filters
          ...
          healthInsurances={props.healthInsurances}
        ...
        <div className="w-full p-2 sm:p-4 md:p-10 grow overflow-x-hidden flex flex-col items-center gap-10 relative">
          <p className="absolute invisible top-[-4rem]" ref={ref}>
            q
          </p>
          {props.doctors.items?.length === 0 ? (
            <h2 className="text-xl absolute top-1/2 left-1/2 translate-x-[-50%] translate-y-[-50%] text-center">
          ...
          {props.doctors.items?.length > 0 && (
            <div className="bg-primary p-4 w-full flex justify-center items-center rounded-md shadow-md">
              <Pagination
                page={Number(props.doctors.currentPage)}
                sx={{
                ...
                count={props.doctors.totalPages}
        ...
        <Fab
          ...
          variant="extended"
          className="z-0 bg-secondary hover:bg-[#4F4F4F] absolute bottom-4 right-8 text-white xl:hidden"
        >
...
const Filters: React.FC<FiltersProps> = (props) => {
  ...
  const filtersForm = useFormik({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      ...
      valuesCpy.orderBy =
        values.orderByField === "none"
          ? ""
          : `${values.orderByField}_${orderDirection}`;
      router.push(
        `${router.pathname}?${new URLSearchParams(valuesCpy).toString()}`
      );
  ...
  return (
    <div
      className={`h-full bg-white transition-[right] duration-300 ease-in-out ${
        props.openedFilters ? "right-0" : "right-[-63rem] md:flex flex-col"
      } 
    ...
    >
      <span className="flex items-center justify-center">
        <h2 className={`${robotoBold.className} text-primary text-xl my-4`}>
          Filtrar
        </h2>
        <IconButton
          onClick={() => props.setOpenedFilters(false)}
      ...
      <div className="px-4 h-full w-full">
        ...
        <form
          ...
          className="py-3 flex flex-col gap-1 md:gap-8"
        >
          <Input
            name="name"
            onChange={filtersForm.handleChange}
            startadornment={<FaUserDoctor color={theme.palette.primary.main} />}
            fullWidth
            label="Nombre"
          />
          <Autocomplete
          ...
          <Autocomplete
            ...
            options={props.healthInsurances.map((hi: any) => ({
              id: hi.id,
              label: hi.name,
            }))}
            renderInput={(params: any) => (
